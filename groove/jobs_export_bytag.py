...
def run(payload: dict) -> bytes:
    ...
    logger.info(
        "export_bytag: %d samples, preserve_paths=%s, export_samples=%s, export_metadata=%s",
        len(samples), preserve_paths, export_samples, export_metadata,
    )
    ...
    with zipfile.ZipFile(zip_buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        # errors.txt — only if there were errors packing samples
        if error_paths:
            err = '\n'.join(error_paths) + '\n'
    ...
    zip_buf.seek(0)
    logger.info(
        "export_bytag: done — %d in selection, %d errors",
        len(csv_rows), len(error_paths),
    )
    return zip_buf.read()
